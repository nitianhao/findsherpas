...
const SUGGEST_MAP: Record<string, string[]> = {
  ...
  linkedin_url: ['linkedin', 'linkedin url', 'li'],
...
function suggestMapping(csvHeaders: string[]): Record<string, string> {
  ...
  for (const header of csvHeaders) {
    ...
    for (const [field, aliases] of Object.entries(SUGGEST_MAP)) {
...
export function ContactCSVImportForm() {
  ...
  const handleFileUpload = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      if (!content) { setError('File is empty.'); return; }
      const parsed = parseCSV(content);
      if (parsed.headers.length === 0) { setError('No headers found in CSV file.'); return; }
      if (parsed.rows.length === 0) { setError('No data rows found in CSV file.'); return; }
  ...
  const handleMappingChange = useCallback((csvHeader: string, value: string | null) => {
    setMapping((prev) => {
      const next = { ...prev };
      if (!value || value === SKIP_VALUE) { delete next[csvHeader]; }
      else { next[csvHeader] = value; }
      return next;
    });
  }, []);
  ...
  const canProceed = hasNameMapping && hasEmailMapping;
      ...
      <div className="flex items-center gap-2 text-sm text-muted-foreground">
        {[1, 2, 3, 4].map((s) => (
          <div key={s} className="flex items-center gap-2">
            <span className={`flex h-6 w-6 items-center justify-center rounded-full text-xs font-medium ${step >= s ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'}`}>
              {s}
            </span>
      ...
      {step === 1 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Select a CSV file with contact data. Company names will be matched to existing companies by name.
            </CardDescription>
          </CardHeader>
          <CardContent>
      ...
      {step === 2 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Match each CSV column to a contact field. {fileName && `File: ${fileName}`} ({rows.length} rows)
            </CardDescription>
          </CardHeader>
          <CardContent>
          ...
          <CardFooter className="flex flex-col items-start gap-3">
            ...
            {!canProceed && (
              <p className="text-sm text-muted-foreground">
                You must map columns for both &quot;Full Name&quot; and &quot;Email&quot; to proceed.
              </p>
            )}
          </CardFooter>
        </Card>
      ...
      {step === 4 && result && (
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="rounded-lg bg-green-50 p-4 dark:bg-green-950/30">
                ...
                <p className="text-sm text-green-600 dark:text-green-500">Contacts imported</p>
              </div>
              <div className="rounded-lg bg-orange-50 p-4 dark:bg-orange-950/30">
                ...
                <p className="text-sm text-orange-600 dark:text-orange-500">Rows skipped</p>
              </div>
            </div>
          ...
          <CardFooter className="flex gap-3">
            ...
            <Button onClick={() => { window.location.href = '/crm/contacts'; }}>
              View Contacts
            </Button>
          </CardFooter>
        </Card>
